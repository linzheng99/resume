...
const config: Config = {
  ...
  skillDetails: [
    ...
    // '熟悉 NextJs 并使用独立开发了一个项目管理系统。', // 包括但不限于 CURD 操作、中间件配置、模块化设计等
    '熟悉 Eslint 基本配置和良好且严格的编码习惯。',
    '熟练掌握多端移动端适配，确保跨设备良好用户体验。',
      ...
      projects: [
        {
          ...
          actions: [
            ...
            {
              details: '对 Element Plus 组件库进行二次封装，并且使用 Pinia 作为状态管理库，对复杂设备管理界面进行重构，提高开发效率以及维护性，减少重复代码。'
            },
            {
              details: '通过高德地图 JSAPI 实现设备定位功能，并结合 WebSocket 通信技术，实时同步更新设备定位数据，确保信息的及时性和准确性。'
            },
            {
              details: '使用 Echarts 可视化开发数据大屏，对图表进行自适应渲染，提升数据展示的效率和用户体验。'
            },
            {
              details: '对图片等资源进行预加载，避免页面加载过慢或出现资源缺失的情况，确保页面加载的流畅性和完整性。'
            }
          ],
          result: {
            title: '结果',
      ...
      projects: [
        {
          ...
          actions: [
            {
              details: '使用 React + TypeScript 作为主框架。'
            },
            {
              details: '使用 react-router 实现路由管理，并使用 redux 进行状态管理。'
            },
            {
              details: '使用 React-Big-Calendar 实现日历视图，管理成员任务安排。'
            },
            {
              details: '集成实时数据分析和可视化功能，利用 ECharts 等图表库展示项目进度和效率分析。'
            }
          ],
          result: {
            title: '结果',
            details: '这个项目为团队提供了一个实用、高效的项目管理工具。帮助公司提升开发效率，减少沟通成本。'
          }
        }
      ]
    },
